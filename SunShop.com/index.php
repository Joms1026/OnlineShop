...
if(isset($_POST["register"]))
			...
			if ($_POST["pass"]==$_POST["cpass"])
			{
				...
				if ($hasil==0)
				{
					if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
						...
						// 	<br>
						// 	<strong>Terima Kasih Telah Mendaftar Sun Shop</strong><br>
						...
						// 	$mail->Username   = "sunshopaplinproyek@gmail.com"; // username email akun
						// 	$mail->Password   = "proyekaplin";        // password akun

						// 	$mail->SetFrom('sunshopaplinproyek@gmail.com', 'Sun Shop');
	...
	{
		$userlogin = $_POST["luser"];
        $passlogin = $_POST["lpass"];
        $sqllogin = "select * from user where email='$userlogin'";
        $result = $conn->query($sqllogin);
    
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                if (password_verify($passlogin, $row["PASSWORD_USER"])) {
                    if ($row["ROLE"] == "0") {
